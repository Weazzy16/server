           ...
           <div class="box-column">
                ...
                <div class="box-flex mt-8">
                    {#if item.isEvacuation}
                        <div on:keypress={() => {}} on:click={() => onEvacuation(item.number)} class="fractions__main_button w-134 mr-10">Эвакуировать</div>
                    {/if}
                    {#if item.isGps}
                        <div on:keypress={() => {}} on:click={() => onGps(item.number)} class="fractions__main_button w-134">Отметка в GPS</div>
                    {/if}
                </div>
               {#if isVehicleUpdateRank}
                    <div on:keypress={() => {}} on:click={() => onUpdateRank("Список рангов", "fractionsicon-parking", "Select the rank from which this transport will be available.", 'updateVehicleRank', item.number)} class="fractions__main_button w-100 mt-8">Настройка доступа</div>
               {/if}
               {#if isSellVehicle}
                    <div on:keypress={() => {}} on:click={() => onSellCar(item.number)} class="fractions__main_button w-100 mt-8">Продать</div>
               {/if}
           </div>
        {/each}
    </div>
