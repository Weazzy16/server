...
<div class="box-between">
    <div class="fractions__stats_subtitle gray hover mt-20 large" on:keypress={() => {}} on:click={() => onSelectDepartment()}>&#x2190; {translateText('player1', 'Вернуться к общей информации')}</div>
    <div class="box-flex">
        {#if settings.isLeader}
        <div class="fractions__main_button extra mr-10" on:keypress={() => {}} on:click={() => onSettings (department)}>{translateText('player1', 'Настройки')}</div>
        {/if}
        {#if settings.deleteDepartment}
        <div class="fractions__main_button extra" on:keypress={() => {}} on:click={() => onDepartmentDelete(department)}>{translateText('player1', 'Удалить отряд')}</div>
        {/if}
    </div>
</div>
    ...
    <div class="fractions__main_box w-600">
        ...
        <div class="fractions__main_grid">
            ...
            <div class="fractions__main_element">
                ...
                <div class="fractions__stats_subtitle mw-161">{department.playerCount} людей.</div>
            </div>
            <div class="fractions__main_element">
    ...
    <div class="fractions__main_box w-317">
        ...
        <div class="box-column">
            {#if department.isSettings}
                <div class="fractions__main_button h-40 mb-16" on:keypress={() => {}} on:click={onUpdate}>{translateText('player1', 'Изменить данные отряда')}</div>
            {/if}
            {#if settings.isLeader || department.myRank === 3}
                <div class="fractions__main_button h-40" on:keypress={() => {}} on:click={onSquadRanks}>{translateText('player1', 'Назначить руководство')}</div>
            {/if}
        </div>
    </div>
    {/if}
