    ...
    <div class="fractions__main_scroll big">
        {#each members.filter(el => filterCheck(el.name, searchText)) as item}
            {#if playerUuid !== item.uuid}
                <div class="fractions__scroll_element small hover" on:keypress={() => {}} on:click={() => onSelectMember (item)}>
                    ...
                    <div class="box-flex hidden">
                        <div class="fractionsicon-fired" on:keypress={() => {}} on:click={(event) => onDeletePlayer (event, item)}></div>
                    </div>
                    {:else}
