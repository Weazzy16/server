<script>
    ...
    const onSaveSetting = (slogan, value, discord, color) => {
        ...
        if (settings.salary != value) {
            if (0 > value || value > 3) {
                window.notificationAdd(4, 9, "The collection cannot be less than 0% and not be higher than 3%", 5000);
                return;
            }
        }
    ...
    const onCopyDiscord = () => {
        ...
        window.notificationAdd(4, 9, `You copied the link to Discord. Insert it into your usual browser :)`, 7000);
    }
</script>
...
<div class="fractions__main_box w-520">
    <div class="fractions__main_head box-between">
        ...
        {#if settings.isLeader}
            <div class="fractions__main_button mini" on:keypress={() => {}} on:click={onOpenPopupSetting}>{translateText('player1', 'Настроить')}</div>
        {/if}
    </div>
    ...
    <div class="fractions__main_grid">
        ...
        <div class="fractions__main_element">
            ...
            <div class="fractions__stats_subtitle">
                <div class="box-flex">
                    {settings.discord}
                    <span class="fractionsicon-copy" on:keypress={() => {}} on:click={onCopyDiscord}></span>
                </div>
            </div>
        </div>
    </div>
