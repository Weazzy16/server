...
{#if isSelector}
    <div class="fractions__main_box {mainScroll}">
        ...
        <div class="box-flex">
            <div class="fractions__selector" class:active={!isWeaponAccess} on:keypress={() => {}} on:click={() => isWeaponAccess = false}>
                {translateText('player1', 'Доступы')}
            </div>
            {#if access.findIndex(a => a.isWeaponAccess) !== -1}
                <div class="fractions__selector" class:active={isWeaponAccess} on:keypress={() => {}} on:click={() => isWeaponAccess = true}>
                    Создать
                </div>
            {/if}
        </div>
            ...
            {#each access as item, index}
                <div class="fractions__scroll_element">
                    ...
                    <div class="fractions__access_box">
                        <div class="fractions__access_element decline" class:active={item.type === accessType.Remove} on:keypress={() => {}} on:click={() => onUpdateAccess (item.id, accessType.Remove)}><div class="fractionsicon-decline"></div></div>
                        {#if isSkip}
                            <div class="fractions__access_element neutral" class:active={item.type === accessType.Skip} on:keypress={() => {}} on:click={() => onUpdateAccess (item.id, accessType.Skip)}><div class="fractionsicon-neutral"></div></div>
                        {/if}
                        <div class="fractions__access_element accept" class:active={item.type === accessType.Add} on:keypress={() => {}} on:click={() => onUpdateAccess (item.id, accessType.Add)}><div class="fractionsicon-check"></div></div>
                    </div>
                </div>
            {/each}
        </div>
