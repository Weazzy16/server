...
<div class="fractions__main_box w-215">
    ...
    {#if stock.isGunAccessStock}
        {#if stock.isGunAccessStock}
            <div class="fractions__main_button medium" on:keypress={() => {}} on:click={updateGunStock}>{stock.isGunStock ? translateText('player1', 'Закрыть склад') : translateText('player1', 'Открыть склад')}</div>
        {/if}

        {#if stock.isAccessStock}
            <div class="fractions__main_button medium" on:keypress={() => {}} on:click={updateStock}>{stock.isStock ? translateText('player1', 'Запретить крафт') : translateText('player1', 'Разрешить крафт')}</div>
        {/if}
    {:else}
