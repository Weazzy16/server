...
<div class="box-flex">
    <div class="fractions__button_small" class:disabled={selectPage === 1} on:keypress={() => {}} on:click={() => setPage(selectPage - 1)}>
        &lt;
    </div>
    {#if pageCount.length > 0}
        <div class="fractions__button_small" class:active={selectPage === 1} on:keypress={() => {}} on:click={() => setPage(1)}>
            1
        </div>
    {/if}
    ...
    {#each buttons as n}
        {#if n > 0 && n < pageCount.length - 1}
            <div class="fractions__button_small" class:active={selectPage === n + 1} on:keypress={() => {}} on:click={() => setPage(n + 1)}>{n + 1}</div>
        {/if}
    {/each}
    ...
    {#if pageCount.length > 1}
        <div class="fractions__button_small" class:active={selectPage === pageCount.length} on:keypress={() => {}} on:click={() => setPage(pageCount.length)}>
            {pageCount.length}
        </div>
    {/if}
    <div class="fractions__button_small" class:disabled={selectPage === pageCount.length} on:keypress={() => {}} on:click={() => setPage(selectPage + 1)}>
        &gt;
    </div>
</div>