...
{:else}
    <div class="fractions__stats_subtitle gray hover mt-20 large" on:keypress={() => {}} on:click={() => onSettings()}>&#x2190; {translateText('player1', 'Вернуться к меню управления отрядом')}</div>
            ...
            {#each settings.ranks.filter(el => filterCheck(el.name, searchText)) as item, index}
                <div class="fractions__scroll_element hover p-20">
                    ...
                    <div class="box-flex hidden">
                        <div class="fractions__main_button black" on:keypress={() => {}} on:click={() => onSelectRank (item)}>{translateText('player1', 'Редактировать')}</div>
                    </div>
                </div>
            {/each}
        </div>
