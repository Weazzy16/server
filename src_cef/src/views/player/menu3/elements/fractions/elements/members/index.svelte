<script>
    ...
    const onPlayerDelete = (item) => {
        ...
        setPopup ("Confirm", {
            ...
            text: `You really want to fire '${item.name}'?`,
            button: 'Fire',
...
{#if selectMember}
    <div class="fractions__header">
        ...
        {#if selectMember !== member}
            <div class="box-flex">
                <!--<div class="fractions__main_button extra mr-10" on:keypress={() => {}} on:click={onPlayerAddScore}>{translateText('player1', 'Выдать очки')}</div>-->
                {#if settings.unInvite}
                <div class="fractions__main_button extra" on:keypress={() => {}} on:click={() => onPlayerDelete (selectMember)}>{translateText('player1', 'Уволить')}</div>
                {/if}
            </div>
        {/if}
    </div>
    {#if selectMember !== member}
        <div class="fractions__stats_subtitle gray hover mt-20 large" on:keypress={() => {}} on:click={() => onSelectMember (member)}>&#x2190; {translateText('player1', 'Вернуться к общей информации')}</div>
    {/if}
            ...
            <div class="fractions__main_box w-477">
                ...
                <div class="fractions__main_grid">
                    <div class="fractions__main_element">
                        ...
                        <div class="fractions__stats_subtitle">{moment.duration(selectMember.todayTime, "minutes").format("h[h.] m[m.]")}</div>
                    </div>
                    <div class="fractions__main_element">
                        <div class="fractions_stats_title">{translateText('player2', 'Онлайн за неделю')}:</div>
                        <div class="fractions__stats_subtitle">{moment.duration(selectMember.weekTime, "minutes").format("h[h.] m[m.]")}</div>
                    </div>
                    <div class="fractions__main_element">
                        <div class="fractions_stats_title">{translateText('player2', 'Онлайн за месяц')}:</div>
                        <div class="fractions__stats_subtitle">{moment.duration(selectMember.monthTime, "minutes").format("h[h.] m[m.]")}</div>
                    </div>
                    <div class="fractions__main_element">
                        <div class="fractions_stats_title">{translateText('player2', 'Общий онлайн')}:</div>
                        <div class="fractions__stats_subtitle">{moment.duration(selectMember.totalTime, "minutes").format("h[h.] m[m.]")}</div>
                    </div>
                    <div class="fractions__main_element">
