<script>
    ...
    const onReprimand = () => {
        setPopup ("Input", {
            ...
            inputs: [
                {
                    title: "Flot of reprimand",
                    placeholder: "Enter the headline",
                    maxLength: 20
                },
                {
                    title: "The text of the reprimand",
...
<div class="box-between mt-20">
    <div class="box-column">
        ...
        <div class="box-between">
            {#if settings.setRank}
            <div class="fractions__main_button extra" on:keypress={() => {}} on:click={() => onUpdateRank ("Список рангов", "fractionsicon-members", "Выберите ранг", 'updatePlayerRank', selectMember.uuid)}>
                {translateText('player1', 'Сменить ранг')}
            </div>
            {/if}
            {#if settings.isLeader || (member.departmentRank === 3)}
            <div class="fractions__main_button extra" on:keypress={() => {}} on:click={onUpdateDepartment}>
                {translateText('player1', 'Сменить отряд')}
            </div>
            {/if}
            {#if settings.reprimand}
            <div class="fractions__main_button extrasmall" on:keypress={() => {}} on:click={onReprimand}>
