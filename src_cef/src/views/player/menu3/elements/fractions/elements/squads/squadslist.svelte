        ...
        {#each departments.filter(el => filterCheck(el.name, searchText)) as item}
            <div class="fractions__scroll_element hover p-20">
                ...
                <div class="box-flex hidden">
                    <div class="fractions__main_button black" on:keypress={() => {}} on:click={() => onSelectDepartment (item)}>{translateText('player1', 'Информация')}</div>
                    {#if settings.deleteDepartment}
                    <div class="fractionsicon-fired"  on:keypress={() => {}} on:click={() => onDepartmentDelete (item)}></div>
                    {/if}
                </div>
            </div>
        {/each}
