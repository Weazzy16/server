<script>
    ...
    const updateBoard = () => {
        setPopup ("Input", {
            ...
            inputs: [
                {
                    title: "Новое имя новости",
                    placeholder: "Введите новое имя",
                    maxLength: 20,
                    value: board.title
                },
                {
                    title: "Новые текст новости",
...
<div class="box-column">
    ...
    <div class="box-flex mt-8">
        {#if board.uuid === $charUUID || (typeof board.uuid !== "undefined" && settings.editAllTabletWall)}
            <div class="fractions__main_button news" on:keypress={() => {}} on:click={deleteBoard}>{translateText('player1', 'Удалить')}</div>
        {/if}
        {#if board.uuid === $charUUID || (typeof board.uuid !== "undefined" && settings.editAllTabletWall)}
            <div class="fractions__main_button news" on:keypress={() => {}} on:click={updateBoard}>{translateText('player1', 'Редактировать')}</div>
        {/if}
        {#if settings.tableWall}
            <div class="fractions__main_button news" on:keypress={() => {}} on:click={addBoard}>{translateText('player1', 'Написать')}</div>
        {/if}
    </div>
</div>
