<script>
    ...
    const getMaxStakcItems = (item, itemInfo) => {
        ...
        if (itemInfo.Stack <= 1) {
						...
						ItemsData[arrayName].forEach((i) => {
							if (!success && ((ammoType && ammoType == WeaponsAmmoTypes[i.ItemId]) || item.ItemId === i.ItemId)) {
                                if (++countItems >= maxItemCount) {
                                    success = -1;
                                    window.notificationAdd(4, 9, `It is impossible to take ${itemInfo.Name}, because the inventory already has a weapon of this type.`, 3000);
                                }
							}                   
						})
					}
                    ...
                    if (arrayName !== "other" && arrayName !== "backpack" && arrayName !== "trade" && arrayName !== "with_trade") 
					{
						ItemsData[arrayName].forEach((i) => {
							if (!success && i.ItemId == -9) {                                
                                if (++countItems >= maxItemCount) {
                                    success = -1;
                                    window.notificationAdd(4, 9, `You already have ${itemInfo.Name}`, 3000);
                                }
							}
						})
					}
            ...
            if (Math.round (maxStack) === Math.round (count)) {
                window.notificationAdd(4, 9, `There is no place for ${itemInfo.Name}, You can have a maximum with you - ${maxStack} PC. | You ${count} PC.`, 3000);
                return -1;
            }
    ...
    const getName = (Item, arrayName) => {
        ...
        if (Item.ItemId == -9 && arrayName !== "accessories") {
            name += `<span>State: ${Item.Data}</span>`;
        }
        ...
        <div class="box-item-info" style="top: {boxInfoTop}px;left: {boxInfoLeft}px;" bind:this={boxItemInfo}>
            <div class="bg-info">
                ...
                {#if wMaxHP [infoItem.ItemId] && infoItem.Data && infoItem.Data.split("_") && infoItem.Data.split("_").length > 1 && infoItem.Data.split("_")[1] != undefined}
                    <div class="type" style="color: #FFA07A"> | State: {weaponCondition (infoItem.Data.split("_")[1], wMaxHP [infoItem.ItemId])}% </div>
                {/if}
<!--            <span class={`${window.getItem (infoItem.ItemId).Icon} icon`} />
                ...
                {#if OtherInfo.Id == otherType.Tent && infoItem.Price}
                    <div class="type" style="color: #FFA07A">Цена за 1 изд.. $ {format("money", infoItem.Price)}</div>
                {/if}
            </div>
        </div>
    {/if}
    ...
    {#if selectItem.use === stageItem.useItem && selectItem.tent && !OtherInfo.IsMyTent}
        <div bind:this={boxPopup} class="box-stack" 
            ...
            {/if}
            <div class="btn-slap" on:keypress={() => {}} on:click={onBuy}>Buy</div>
            <div class="box-cancel"><span class='icon inv-close' on:keypress={() => {}} on:click={e => selectItem = defaulSelectItem} /></div>
        </div>
    ...
    <div bind:this={boxPopup} class="box-use" 
        ...
        {#if OtherInfo.Id == otherType.Tent && OtherInfo.IsMyTent && selectItem.arrayName === "other"}
            <div class="item" on:keypress={() => {}} on:click={onTransfer}><span class="inventoryicons-backpack inventoryicon-use"></span>{translateText('player1', 'забрать')}</div>
        {:else}
            {#if getItemsUse (selectItem) !== false}
                <div class="item" on:keypress={() => {}} on:click={onUseItem}>{@html getItemsClickInfo (selectItem)}</div>
            {/if}
            {#if getToPut (selectItem.ItemId) !== false && getDropItem (selectItem.arrayName, selectItem.ItemId) !== false && selectItem.arrayName === "inventory"}
                <div class="item" on:keypress={() => {}} on:click={onToPut}><span class="inventoryicons-near inventoryicon-use"></span>{translateText('player1', 'поставить')}</div>
            {/if}
            {#if OtherInfo.Id != otherType.Tent && (OtherInfo.Id > otherType.None || (maxSlotBackpack > 0 && ItemsData["backpack"].length) || tradeInfo.Active === true) && selectItem.arrayName !== "fastSlots" && selectItem.arrayName !== "accessories"}
                <div class="item" on:keypress={() => {}} on:click={onTransfer}><span class="inventoryicons-hand inventoryicon-use"></span>{(selectItem.arrayName === "other" || selectItem.arrayName === "backpack" || selectItem.arrayName === "trade") ? translateText('player1', 'взять') : translateText('player1', 'передать')}</div>
            {:else if OtherInfo.Id == otherType.Tent && OtherInfo.IsMyTent && (selectItem.arrayName === "inventory" || selectItem.arrayName === "backpack" )}
                <div class="item" on:keypress={() => {}} on:click={onTransfer}><span class="inventoryicons-shop-cart inventoryicon-use"></span>{translateText('player1', 'продать')}</div>
            {/if}
            {#if selectItem.Count > 1 && selectItem.arrayName !== "fastSlots"}
                <div class="item" on:keypress={() => {}} on:click={e => {
                    ItemStack = 0;
            ...
            {#if getDropItem (selectItem.arrayName, selectItem.ItemId) !== false}
                <div class="item" on:keypress={() => {}} on:click={onDropItem}><span class="inventoryicons-drop inventoryicon-use"></span>{translateText('player1', 'выбросить')}</div>
            {/if}
        {/if}
    </div>
    {:else if selectItem.use === stageItem.useItem && ItemStack !== -1}
    <div bind:this={boxPopup} class="box-stack" 
        ...
        <div class="box-between" style="width: 100%">
            <div class="btn-slap" on:keypress={() => {}} on:click={onStack}>{!ItemStack ? translateText('player1', 'Разделить') : ItemStack == 1 ? translateText('player1', 'Выбросить') : translateText('player1', 'Передать')}</div>
            <div class="box-cancel" on:keypress={() => {}} on:click={e => ItemStack = -1}>{translateText('player1', 'Отмена')}</div>
        </div>
    </div>
    {/if}
            ...
            <div class="box-width-356 trade">
                ...
                {#if tradeInfo.Active}
                    <div class="btn-box-trade" on:mouseenter={e => mainInventoryArea = true} on:mouseleave={e => mainInventoryArea = false}>
                        ...
                        <div class="btn-trade" style="opacity: {tradeInfo.YourStatusChange ? "0.5" : 1}" on:keypress={() => {}} on:click={TradeSelect}>{translateText('player1', 'Подтвердить')}</div>
                        {:else}
                        <div class="btn-trade" style="opacity: {tradeInfo.YourStatus ? "0.5" : 1}" on:keypress={() => {}} on:click={TradeSelect}>{translateText('player1', 'Готов')}</div>
                        {/if}
                        <div class="btn-trade red" style="opacity: {!tradeInfo.YourStatusChange && !tradeInfo.YourStatus ? "0.5" : 1}" on:keypress={() => {}} on:click={TradeCancel}>{translateText('player1', 'Отменить')}</div>
                    </div>                    
                {/if}
            </div>
        </div>
        ...
        <div class="e-player">
            <div class="box-absolute-player" style="position: absolute;top: {moveBlock["inventory"][0]}px;left: {moveBlock["inventory"][1]}px;" on:mousedown={(event) => handleMouseDownBlock(event, "box-absolute-player", "inventory")}>
                <div class="box-column" style="opacity: {invOpacity}">
                    <div class="inventory__title">Карман</div>
                    <div class="inventory__text">Это те вещи, которые лежат у вас в кармане.</div>
                </div>
                ...
                {#if maxSlotBackpack > 0 && ItemsData["backpack"].length && ItemsData["backpack"][0] !== undefined && ItemsData["backpack"][0].use !== undefined} <!--  || (ItemsData["backpack"][0].use === false && OtherInfo.Id === otherType.None) -->
                    <div style="opacity: {invOpacity}">
                        <div class="box-column">
                            <div class="inventory__title">Рюкзак</div>
                            <div class="inventory__text">Это те вещи, которые находятся в вашем рюкзаке.</div>
                        </div>
        ...
        <div class="box-width-457 accessories box-column" style="position: {moveBlock["accessories"][0] == null ? "unset" : "unset"}">
            <div class="box-accessories" style="top: {moveBlock["accessories"][0]}px;left: {moveBlock["accessories"][1]}px;" on:mousedown={(event) => handleMouseDownBlock(event, "box-accessories", "accessories")} on:mouseup={setAccessories} on:mouseenter={e => mainInventoryArea = true} on:mouseleave={e => mainInventoryArea = false}>
                ...
                <div class="inventory__accessories">
                    <div class="box-column">
                        <div class="inventory__title">Снаряжение</div>
                        <div class="inventory__text">Это те вещи, которые на тебе надеты.</div>
                    </div>
