<script>
    ...
    import jobs from 'json/jobs.js';
      import skillDescriptions from 'json/skillDescriptions.js';
...
function openBanHistory() {
 ...
 function setMarker() {
    // Передаём координаты на клиент
    mp.trigger('client:setHouseWaypoint',
      selectCharData.housePosition.x,
      selectCharData.housePosition.y
    );
  }
  function setMarkerBiz() {
    // Передаём координаты на клиент
    mp.trigger('client:setBizWaypoint',
      selectCharData.BizEnterPoint.x,
      selectCharData.BizEnterPoint.y
    );
  }
  const bizFolderMap = {
    0: 'item_shops',
  ...
  // 2) Маппинг типов → название (русск.)
  const bizNameMap = {
    0: 'Магазин 24/7',
    1: 'АЗС',
    2: 'Автосалон',
    ...
    8: 'Татусалон',
    9: 'Барбершоп',
    10: 'Los Santos Customs',
    11: 'Мойка',
    12: 'Автосалон'
  };

  // реактивно вычисляем нужные значения
  $: bizFolder = bizFolderMap[selectCharData.BizType] ?? 'item_shops';
  $: bizTitle  = bizNameMap[selectCharData.BizType]
                  ? `${bizNameMap[selectCharData.BizType]} #${selectCharData.BizId}`
                  : `Бизнес #${selectCharData.BizId}`;

                  function calcWidth(job) {
    if (job.currentLevel >= 5) return 100;
    const rankPct = (job.currentLevel / 5) * 100;
    const innerPct = (job.current / job.nextLevel) * 20;
    return rankPct + innerPct;
  }

  function getProgressPercent(job) {
    return Math.round((job.current / job.nextLevel) * 100);
  }
</script>
...
{#if selectView === "Osnovnoe"}
    ...
    <span class="address-info">
        ...
        <div class="block">
            <h1>Организация</h1>
            <pre>{JSON.stringify(selectCharData, null, 2)}</pre>
...
{#if selectView === 'Skills'}
  {#if selectCharData.jobSkillsInfo}
    <div class="skills">
  <div class="wrapper column-block full-width">
    <div class="category column-block ">
        <!-- Заголовок категории -->
        <h3>Профессии</h3>

        <!-- Обёртка карточек -->
        <div class="card-wrapper column-block full-height">
            {#each selectCharData.jobSkillsInfo as job}
              <!-- Каждая карточка в своём контейнере -->
              <div class="card">
              <!-- Заголовок карточки -->
              <div class="card-heading">
                <div class="top">
                  ...
                  {#if skillDescriptions[job.name]}
          <p class="description">{skillDescriptions[job.name]}</p>
                  {:else}
                    <p class="description description--empty">Нет описания</p>
                  {/if}
                  </div>
                ...
                <div class="progress">
                  ...
                  {#if job.currentLevel >= 0}
                      <!-- Ранг -->
                      <span>{job.currentLevel + 1} ранг</span>
                    {:else}
...
{@const modelKey          = item[1].toLowerCase()}
  {@const baseSpeed         = speedLimits[modelKey] ?? 0}
  ...
  {@const maxSpeed          = baseSpeed + engineBonus + gearboxBonus + turboBonus}
            ...
            <div class="blockcars">
                <div class="headblock">
                    <div class="km-block">
                        ...
                        <span class="price-text">{item[15].toLocaleString('ru-RU')} км</span>
                    </div>
            ...
            {scrapPrice > 0 ? `$${scrapPrice.toLocaleString('ru-RU')}` : '—'}
                ...
                <div class="infocar">
                    ...
                    <div class="fuel">
                       ...
                       <span
    class="fuel"
    style="background-color: {fuelColors[item[10] || 'Regular']};"
  >
    {item[10] || 'Regular'}
  </span>                                    
                    </div>
      ...
      <div class="row">
        ...
        <div class="item2">
          <img
            ...
            alt="Max Speed"
          />
          <span class="name">{maxSpeed.toLocaleString('ru-RU')} км/ч</span>
...
{#if selectCharData.houseId}
	...
	<path d="M13.9294 0.0681575C13.885 0.0245144 13.8247 0 13.7619 0C13.6991 0 13.6389 0.0245144 13.5944 0.0681575L12.7151 0.932708H7.1878L0.245199 7.81782C0.0881982 7.97223 0 8.18162 0 8.39995C0 8.61828 0.0881982 8.82767 0.245199 8.98208L5.10251 13.7577C5.18032 13.8345 5.27279 13.8954 5.37461 13.937C5.47643 13.9786 5.5856 14 5.69586 14C5.80611 14 5.91528 13.9786 6.0171 13.937C6.11892 13.8954 6.21139 13.8345 6.2892 13.7577L13.0518 6.92939V1.26288L13.9311 0.398333C13.9531 0.376576 13.9705 0.350762 13.9823 0.322373C13.9941 0.293985 14.0002 0.26358 14 0.232903C13.9998 0.202227 13.9935 0.171882 13.9814 0.143611C13.9693 0.11534 13.9516 0.0896982 13.9294 0.0681575ZM10.4397 4.66592C10.205 4.66592 9.97555 4.59749 9.7804 4.46929C9.58525 4.34109 9.43315 4.15887 9.34333 3.94567C9.25351 3.73248 9.23001 3.49789 9.2758 3.27157C9.32159 3.04524 9.43461 2.83735 9.60057 2.67418C9.76653 2.51101 9.97798 2.39989 10.2082 2.35487C10.4384 2.30985 10.677 2.33296 10.8938 2.42127C11.1107 2.50957 11.296 2.65912 11.4264 2.85098C11.5568 3.04285 11.6264 3.26843 11.6264 3.49919C11.6262 3.80856 11.5011 4.10519 11.2786 4.32395C11.0561 4.54271 10.7544 4.6657 10.4397 4.66592Z"/>
              ...
              <span class="price" >{selectCharData.housePrice.toLocaleString('ru-RU')} $</span>
        ...
        <div class="footer" >
          <button class="gps-icon" on:click={setMarker}>
    ...
    <path d="M10.667,1.667a7,7,0,0,0-7,7c0,5.667,7,11,7,11s7-5.333,7-11a7,7,0,0,0-7-7Zm0,10.333A3.333,3.333,0,1,1,14,8.667,3.333,3.333,0,0,1,10.667,12Z" transform="translate(-3.667 -1.667)"/>
          ...
          <div >
            <h3 >{selectCharData.area}</h3>
            <p >{selectCharData.street}</p>
          </div>
        </div>
      </div>
    </div>
   {/if}
    {#if selectCharData.BizId}
      ...
      <div class="card" >
        <img class="img" src="http://cdn.piecerp.ru/cloud/img/panelmenu/main/biz/{bizFolder}/{selectCharData.BizId}.jpg" alt="Бизнес #{selectCharData.BizId}"/>
        <div class="card-body" >
          <div class="header" >
            <div class="property-info" >
              <p >Собственность</p>
	...
	<path d="M13.9294 0.0681575C13.885 0.0245144 13.8247 0 13.7619 0C13.6991 0 13.6389 0.0245144 13.5944 0.0681575L12.7151 0.932708H7.1878L0.245199 7.81782C0.0881982 7.97223 0 8.18162 0 8.39995C0 8.61828 0.0881982 8.82767 0.245199 8.98208L5.10251 13.7577C5.18032 13.8345 5.27279 13.8954 5.37461 13.937C5.47643 13.9786 5.5856 14 5.69586 14C5.80611 14 5.91528 13.9786 6.0171 13.937C6.11892 13.8954 6.21139 13.8345 6.2892 13.7577L13.0518 6.92939V1.26288L13.9311 0.398333C13.9531 0.376576 13.9705 0.350762 13.9823 0.322373C13.9941 0.293985 14.0002 0.26358 14 0.232903C13.9998 0.202227 13.9935 0.171882 13.9814 0.143611C13.9693 0.11534 13.9516 0.0896982 13.9294 0.0681575ZM10.4397 4.66592C10.205 4.66592 9.97555 4.59749 9.7804 4.46929C9.58525 4.34109 9.43315 4.15887 9.34333 3.94567C9.25351 3.73248 9.23001 3.49789 9.2758 3.27157C9.32159 3.04524 9.43461 2.83735 9.60057 2.67418C9.76653 2.51101 9.97798 2.39989 10.2082 2.35487C10.4384 2.30985 10.677 2.33296 10.8938 2.42127C11.1107 2.50957 11.296 2.65912 11.4264 2.85098C11.5568 3.04285 11.6264 3.26843 11.6264 3.49919C11.6262 3.80856 11.5011 4.10519 11.2786 4.32395C11.0561 4.54271 10.7544 4.6657 10.4397 4.66592Z"/>
                ...
                <span class="price" >${selectCharData.BizSellPrice.toLocaleString('ru-RU')}</span>
              </div>
            </div>
            
          </div>
          <div class="footer" >
          <button class="gps-icon" on:click={setMarkerBiz}>
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="18" viewBox="0 0 14 18">
    <path d="M10.667,1.667a7,7,0,0,0-7,7c0,5.667,7,11,7,11s7-5.333,7-11a7,7,0,0,0-7-7Zm0,10.333A3.333,3.333,0,1,1,14,8.667,3.333,3.333,0,0,1,10.667,12Z" transform="translate(-3.667 -1.667)"/>
</svg>
          </button>
          <div >
            <h3 >{selectCharData.bizArea}</h3>
            <p >{selectCharData.bizStreet}</p>
          </div>
        </div>
        </div>
      </div>
