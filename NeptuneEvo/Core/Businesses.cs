    ...
    class BusinessManager : Script
    {
        ...
        public static async void createBusinessCommand(ExtPlayer player, int govPrice, int type, double taxes)
        {
            try
            {
                ...
                MySQL.Query(cmd);
                    Business biz = new Business(lastBizID, "Государство", govPrice, type, products_list, pos, new Vector3(0,0,0), bankID, -1, new List<Order>(), taxes);
                    biz.UpdateLabel();
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Бизнес создан успешно", 3000);
                });
            }
            catch (Exception e)
            {
        ...
        public static void deleteBusinessCommand(ExtPlayer player, int id)
        {
            try
            {
                ...
                else if (ownerCharacterData == null)
                {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (busProductData.ItemId != ItemId.Debug)
                    {
                        ...
                        else if (ItemId.Metaldetector == busProductData.ItemId) Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", busProductData.ItemId, 1, "2250");
                        else if (ItemId.Shovel == busProductData.ItemId) Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", busProductData.ItemId, 1, "2250");
    ...
    public class Business
    {
        ...
        public void Destroy()
        {
            try
            {
                ...
                label = null;
                if(mafiaLabel != null && mafiaLabel.Exists) mafiaLabel.Delete();
                mafiaLabel = null;
