    ...
    class Admin : Script
    {
...

        {
...

            {
                ...
                Trigger.SendToAdmins(2, $"{ChatColors.StrongOrange}[A] Запрос на выдачу лидерки для {player.Name} ({characterData.UUID}) удалён в связи с {text}.");
...

            {
                ...
                Trigger.SendToAdmins(2, $"{ChatColors.StrongOrange}[A] Запрос на смену внешности для {player.Name} ({characterData.UUID}) удалён в связи с {text}.");
        ...
        public static void onPlayerDeathHandler(ExtPlayer player, ExtPlayer entityKiller, uint weapon, Vector3 pos)
        {
            try
            {
                ...
                else
                {
                    ...
                    //message = $"~r~{player.Name}({characterData.UUID}) ~s~умер";
                }
        ...
        public static void giveFreeCase(ExtPlayer player, ExtPlayer target, byte caseid)
        {
            try
            {
                ...
                var targetCharacterData = player.GetCharacterData();
                var targetAccountData = target.GetAccountData();
                if (targetAccountData == null) return;
                if (caseid < 0 || caseid > 2) return;
                Chars.Repository.AddNewItemWarehouse(target, ItemId.Case0 + caseid, 1);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы выдали {target.Name} ({targetCharacterData.UUID}) открытие одного Free Case #{caseid}", 3000);
        ...
        public static void stopServer(ExtPlayer sender, string reason = "Происходит плановая перезагрузка сервера!")
        {
            try
            {
                ...
                var data = sender.GetCharacterData();
                if (data.AdminLVL != 11) return;
        ...
        public static void setPlayerAdminGroup(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                AdminsLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) выдал {target.Name} админ. права 1 уровня", 1, "#FFB833", false);
        ...
        public static void delPlayerAdminGroup(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                {
                    Trigger.SendToAdmins(3, "!{#FF0000}" + $"[A] {player.Name} ({characterData.UUID}) попытался снять {target.Name} ({targetCharacterData.UUID}), который имеет выше уровень администратора.");
                    return;
                }
                if (targetCharacterData.AdminLVL < 1)
                {
                ...
                AdminsLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) снял с {target.Name} админ. права", 1, "#FFB833", false);
        ...
        public static void setPlayerAdminRank(ExtPlayer player, ExtPlayer target, int rank)
        {
            try
            {
                ...
                var characterData = player.GetCharacterData();
                if(characterData.AdminLVL < 10) return;
                ...
                AdminsLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) выдал {target.Name} {rank} уровень админ. прав", 1, "#FFB833", false);
        ...
        public static void setPlayerVipLvl(ExtPlayer player, ExtPlayer target, int rank, ushort days)
        {
            try
            {
                ...
                AdminsLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) выставил {target.Name} статус {Group.GroupNames[rank]} на {days} дней.");
        ...
        public static void setFracLeader(ExtPlayer player, ExtPlayer target, int fractionId)
        {
            try
            {
                ...
                if (fractionData != null)
                {
                    ...
                    {
                        AdminsLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) выдал лидерку({fractionId}) {target.Name} ({targetCharacterData.UUID})");
                    ...
                    else if (characterData.AdminLVL >= 5 || (SetLeaderQueue.ContainsKey(target) && SetLeaderQueue[target].Item1 == fractionId))
                    {
                        if (characterData.AdminLVL <= 4)
                        {
                            ...
                            AdminsLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) подтвердил запрос выдачи лидерки администратора {SetLeaderQueue[target].Item2}");
                            AdminsLog(SetLeaderQueue[target].Item3, $"{SetLeaderQueue[target].Item2} выдал лидерку({fractionId}) {target.Name} ({targetCharacterData.UUID})");
                    ...
                    else
                    {
                        ...
                        Trigger.SendToAdmins(2, "!{#FFB833}" + $"[A] Запрос от {player.Name} ({characterData.UUID}) на выдачу лидерки({fractionId}) для {target.Name} ({targetCharacterData.UUID}). Чтобы подтвердить действие - введите: /setleader {targetCharacterData.UUID} {fractionId}");
                    }
                }
            }
            catch (Exception e)
        ...
        public static void delFracLeader(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) снял лидерку {target.Name} ({targetCharacterData.UUID})");
        ...
        public static void delJob(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (targetCharacterData.WorkID != 0)
                {
                    ...
                    //Chars.Repository.PlayerStats(target);
                    Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) уволил с работы {target.Name} ({targetCharacterData.UUID})");
        ...
        public static void delFrac(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                //Chars.Repository.PlayerStats(target);
                Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) выкинул из фракции {target.Name} ({targetCharacterData.UUID})");
        ...
        public static void giveBankMoney(ExtPlayer player, int bankid, int amount)
        {
            try
            {
                ...
                //if (amount >= 1000000)
                //    Admin.AdminsLog(1, $"[ВНИМАНИЕ] Игрок {data.Holder} получил {amount}$ единой операцией от {player.Name}({characterData.UUID}) (giveBankMoney)", 1, "#FF0000");
                ...
                //     {
                //         Admin.AdminsLog(1, $"[ВНИМАНИЕ] Игрок {target.Name}({targetCharacterData.UUID}) два раза подряд получил по {amount}$ от {player.Name}({characterData.UUID}) (giveBankMoney)", 1, "#FF0000");
        ...
        public static void giveMoney(ExtPlayer player, ExtPlayer target, int amount)
        {
            try
            {
                ...
                Log.Write($"{characterData.FirstName}{characterData.LastName} выдаёт деньги {targetCharacterData.FirstName}{targetCharacterData.LastName}");
            }
            catch (Exception e)
            {
        ...
        public static void mutePlayer(ExtPlayer player, ExtPlayer target, int time, string reason)
        {
            try
            {
                ...
                {
                    Trigger.SendToAdmins(1, $"{ChatColors.Red}[A] {player.Name} ({characterData.UUID}) был снят системой за причину в наказании: {reason}");
                ...
                Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} выдал мут игроку {target.Name}({targetCharacterData.UUID}) на {time} {deTimeMsg}. Причина: {reason}", target);
        ...
        public static void unmutePlayer(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (targetCharacterData.Unmute >= 0)
                {
                    ...
                    Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} снял мут с игрока {target.Name}({targetCharacterData.UUID})");
        ...
        public static void getRb(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"У игрока {target.Name} ({targetCharacterData.UUID}) - {targetRb} RedBucks", 5000);
                AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) узнал количество RB у {target.Name} ({targetCharacterData.UUID}) - {targetRb}");
            }
            catch (Exception e)
            {
        ...
        public static void getVip(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"У игрока {target.Name} ({targetCharacterData.UUID}) - VIP {targetVip} уровня до {targetVipDate} ", 5000);
                AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) узнал VIP-статус {target.Name} ({targetCharacterData.UUID}) - {targetVip}");
            }
            catch (Exception e)
            {
                Log.Write($"getRb Exception: {e.ToString()}");
        ...
        public static void getLogin(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Логин игрока {target.Name} ({targetCharacterData.UUID}) - {targetlogin}", 5000);
                AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) проверил логин {target.Name} ({targetCharacterData.UUID}) - {targetlogin}");
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                switch(type)
                {
                    case 0:
                        ...
                        {
                            Trigger.SendToAdmins(3, $"{ChatColors.Red}[A] {player.Name} ({characterData.UUID}) был снят системой безопасности за превышение лимита наказаний");
                            BanMe(player, 1);
                            return false;
                        }
        ...
        public static void banPlayer(ExtPlayer player, ExtPlayer target, int time, string reason, bool isSBan = false)
        {
            try
            {
                ...
                if (!isSBan && !CommandsAccess.CanUseCmd(player, AdminCommands.Banplayer)) return;
                if (player == target) return;
                int tadmlvl = targetCharacterData.AdminLVL;
                if (tadmlvl == 9)
                {
                    Trigger.SendToAdmins(1, "!{#FF0000}" + $"[A] {player.Name} ({characterData.UUID}) попытался забанить {target.Name} ({targetCharacterData.UUID}).");
                    BanMe(player, 0);
                    return;
                }
                else if(tadmlvl != 0 && tadmlvl >= characterData.AdminLVL)
                {
                    Trigger.SendToAdmins(3, $"{ChatColors.StrongOrange}[A] {player.Name} ({characterData.UUID}) забанил {target.Name} ({targetCharacterData.UUID}) и был забанен системой.");
                ...
                else
                {
...

                    {
                        ...
                        if (isSBan == true) AdminsLog(characterData.AdminLVL, $"{player.Name} забанил игрока {target.Name}({targetCharacterData.UUID}) без лишнего шума навсегда. Причина: {reason}", 1, "#FFB833", false);

                        else Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} заблокировал персонажа {target.Name}({targetCharacterData.UUID}) навсегда. Причина: {reason}", target);
...

                    {
                        ...
                        if (isSBan == true) AdminsLog(characterData.AdminLVL, $"{player.Name} забанил игрока {target.Name}({targetCharacterData.UUID}) без лишнего шума на {time} дней. Причина: {reason}", 1, "#FFB833", false);

                        else Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} заблокировал персонажа {target.Name}({targetCharacterData.UUID}) на {time} дней. Причина: {reason}.", target);
                        ...
                        NAPI.Task.Run(() =>
                        {
                            try
                            {
...

                                {
                                    ...
                                    AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) в оффлайне забанил логин({login}) {target.Name} ({targetCharacterData.UUID}) по причине {reason}");
                                ...
                                else AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) в оффлайне забанил логин({login}) по причине {reason}");
                ...
                Trigger.SetTask(async () =>
                {
                    try
                    {
                        ...
                        NAPI.Task.Run(() =>
                        {
                            try
                            {
                                AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) разбанил логин({login})");
        ...
        public static void hardbanPlayer(ExtPlayer player, ExtPlayer target, int time, string reason)
        {
            try
            {
                if (!CommandsAccess.CanUseCmd(player, AdminCommands.Hardbanda)) return;
                ...
                if (tadmlvl == 9)
                {
                    Trigger.SendToAdmins(1, "!{#FF0000}" + $"[A] {player.Name} ({characterData.UUID}) попытался забанить хардом {target.Name} ({targetCharacterData.UUID}).");
                    BanMe(player, 0);
                    return;
                }
                else if (tadmlvl != 0 && tadmlvl >= characterData.AdminLVL)
                {
                    Trigger.SendToAdmins(3, $"{ChatColors.StrongOrange}[A] {player.Name} ({characterData.UUID}) забанил хардом {target.Name} ({targetCharacterData.UUID}) и был забанен системой.");
                ...
                else
                {
                    ...
                    {
                        Trigger.SendToAdmins(3, "!{#FF0000}" + $"[A] {player.Name} ({characterData.UUID}) попытался забанить хардом {target.Name} ({targetCharacterData.UUID}).");
                        BanMe(player, 0);
                        return;
                    }
                    if (!CheckMe(player, 4)) return;
                    DateTime unbanTime = (time >= 3650) ? DateTime.MaxValue : DateTime.Now.AddDays(time);
                    if (time >= 3650) Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} выдал пожизненный банхаммер игроку {target.Name}({targetCharacterData.UUID}). Причина: {reason}", target);
                    else Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} выдал банхаммер игроку {target.Name}({targetCharacterData.UUID}) на {time} дней. Причина: {reason}", target);
                    ...
                    try
                    {
                        ...
                        NAPI.Task.Run(() =>
                        {
                            try
                            {
                                ...
                                {
                                    Trigger.SendToAdmins(1, "!{#FF0000}" + $"[A] {player.Name} ({characterData.UUID}) попытался забанить {prefix} {name} (offline).");
                                    BanMe(player, 0);
                                    return;
                                }
                                if (targetCharacter.Adminlvl != 0 && targetCharacter.Adminlvl >= characterData.AdminLVL)
                                {
                                    ...
                                    if (login != null && Character.Repository.LoginsBlck.Contains(login))
                                    {
                                        ...
                                        Trigger.SendToAdmins(3, "!{#FF0000}" + $"[A] {player.Name} ({characterData.UUID}) попытался забанить {prefix} {name} (offline).");
                                    ...
                                    Trigger.SendToAdmins(3, $"{ChatColors.StrongOrange}[A] {player.Name} ({characterData.UUID}) забанил {name} в оффлайне и был забанен системой.");
                ...
                Trigger.SetTask(async () => {
                    try
                    {
                        ...
                        NAPI.Task.Run(() =>
                        {
                            try
                            {
                                Trigger.SendToAdmins(1, $"~r~[A] {player.Name} ({characterData.UUID}) снял бан {name}");
                ...
                Trigger.SetTask(async () =>
                {
                    try
                    {
                        ...
                        NAPI.Task.Run(() =>
                        {
                            try
                            {
                                Trigger.SendToAdmins(1, $"~r~[A] {player.Name} ({characterData.UUID}) снял хардбан {name}");
        ...
        public static async void unbanIp(ExtPlayer player, string ip)
        {
            try
            {
                ...
                if (ban != null)
                {
                    ...
                    NAPI.Task.Run(() =>
                    {
                        Trigger.SendToAdmins(1, $"~r~[A] {player.Name} ({characterData.UUID}) разблокировал IP адрес: {ip}");
        ...
        public static void kickPlayer(ExtPlayer player, ExtPlayer target, string reason, bool isSilence)
        {
            try
            {
                ...
                if (target.IsCharacterData())
                {
                    if (targetCharacterData.AdminLVL >= characterData.AdminLVL)
                    {
                        Trigger.SendToAdmins(3, "!{#FF0000}" + $"[A] {player.Name} ({characterData.UUID}) попытался кикнуть {target.Name} ({targetCharacterData.UUID}), который имеет выше уровень администратора.");
                        return;
                    }
                }
                ...
                if (!isSilence)
                {
                    Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} кикнул игрока {target.Name}({targetCharacterData.UUID}). Причина: {reason}", target);
                    GameLog.Admin($"{player.Name}", $"kickPlayer({reason})", $"{target.Name}");
                }
                else
                {
                    Trigger.SendToAdmins(1, "!{#FFB833}" + $"[A] {player.Name} кикнул игрока {target.Name}({targetCharacterData.UUID}) без лишнего шума.");
        ...
        public static void warnPlayer(ExtPlayer player, ExtPlayer target, string reason)
        {
            try
            {
                ...
                if (targetCharacterData.AdminLVL >= characterData.AdminLVL)
                {
                    Trigger.SendToAdmins(3, "!{#FF0000}" + $"[A] {player.Name} ({characterData.UUID}) попытался предупредить {target.Name} ({targetCharacterData.UUID}), который имеет выше уровень администратора.");
                    return;
                }
                if (Main.stringGlobalBlock.Any(c => reason.Contains(c)))
                {
                ...
                Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} выдал предупреждение игроку {target.Name}({targetCharacterData.UUID}). | {targetCharacterData.Warns}/3. Причина: {reason}", target);
        ...
        public static void killTarget(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (targetSessionData == null) return;
                if (!target.IsCharacterData()) return;
                AdminsLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) убил игрока {target.Name} ({targetCharacterData.UUID})", 1, "#636363", hideAdminLevel: 9);
                if (!targetSessionData.DeathData.IsDying) NAPI.Player.SetPlayerHealth(target, 0);
                else Ems.ReviveFunc(target);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы убили игрока {target.Name}({targetCharacterData.UUID})", 3000);
        ...
        public static void reviveTarget(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                int id = target.Value;
                AdminsLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) возродил игрока {target.Name} ({targetCharacterData.UUID})", 2, "#636363");
        ...
        public static void healTarget(ExtPlayer player, ExtPlayer target, int hp)
        {
            try
            {
                ...
                AdminsLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) выставил HP({hp}) {target.Name} ({targetCharacterData.UUID})", 1, "#636363", hideAdminLevel: 9);
        ...
        public static void armorTarget(ExtPlayer player, ExtPlayer target, int ar)
        {
            try
            {
                ...
                if (Chars.Repository.itemCount(target, "inventory", ItemId.BodyArmor) < Chars.Repository.maxItemCount)
                {
                    ...
                    Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) выдал Armor({ar}) {target.Name} ({targetCharacterData.UUID})");
        ...
        public static void checkGodmode(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) проверил на GM {target.Name} ({targetCharacterData.UUID})");
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        if (!target.IsCharacterData()) return;
                        if (target.Health >= targetHealth) Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"У {target.Name} ({targetCharacterData.UUID}) возможно есть GM", 3000);
                        else
                        {
                            Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"У {target.Name} ({targetCharacterData.UUID}) нет GM", 3000);
        ...
        public static void slapPlayer(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) подкинул {target.Name} ({targetCharacterData.UUID})");
        ...
        public static void checkMoney(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) проверил денежные средства {target.Name} ({targetCharacterData.UUID})");
            ...
            try
            {
                ...
                if (targetCharacterData.AdminLVL >= 6)
                {
                    ...
                    if (targetAdminConfig.HideMe && characterData.AdminLVL < targetCharacterData.AdminLVL)
                    {
                        ...
                        Notify.Send(target, NotifyType.Alert, NotifyPosition.BottomCenter, $"{player.Name} ({characterData.UUID}) попытался телепортировать Вас к себе (/metp)", 3000);
                        return;
                    }
                }
                AdminsLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) телепортировал к себе {target.Name} ({targetCharacterData.UUID})", 1, "#636363");
        ...
        public static void freezeTarget(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (characterData == null) return;
                if (!target.IsCharacterData()) return;
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) заморозил {target.Name} ({targetCharacterData.UUID})");
        ...
        public static void unFreezeTarget(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (characterData == null) return;
                if (!target.IsCharacterData()) return;
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) разморозил {target.Name} ({targetCharacterData.UUID})");
        ...
        public static void giveTargetGun(ExtPlayer player, ExtPlayer target, string weapon, string serial)
        {
            try
            {
                ...
                var targetCharacterData = player.GetCharacterData();
                var characterData = player.GetCharacterData();
                if(characterData.AdminLVL < 7) return;
                if (characterData == null) return;
                if (!target.IsCharacterData()) return;
                if (serial.Length != 9)
                {
                ...
                if (characterData.AdminLVL >= 8 || (characterData.AdminLVL <= 7 && (Chars.Repository.ItemsInfo[wType].functionType == newItemType.Weapons || Chars.Repository.ItemsInfo[wType].functionType == newItemType.MeleeWeapons)))
                {
                    ...
                    AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) выдал оружие ({weapon}|{serial}) {target.Name} ({targetCharacterData.UUID})");
        ...
        public static void giveTargetCar(ExtPlayer player, ExtPlayer target, string vehicle)
        {
            try
            {
                ...
                AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) выдал купон на машину ({vehicle}) {target.Name} ({targetCharacterData.UUID})");
        ...
        public static void takeTargetGun(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) забрал все оружия {target.Name} ({targetCharacterData.UUID})");
        ...
        public static void adminSMS(ExtPlayer player, ExtPlayer target, string message)
        {
            try
            {
                ...
                Trigger.SendChatMessage(target, $"{ChatColors.Report}Администратор {player.Name} ({characterData.UUID}): {message}");
                Trigger.SendToAdmins(characterData.AdminLVL, $"{ChatColors.Report}[A][ASMS] {player.Name} ({characterData.UUID}) для {target.Name} ({targetCharacterData.UUID}): {message}");
        ...
        public static void checkKill(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) проверил данные последней смерти {target.Name} ({targetCharacterData.UUID})");
        ...
        public static void adminChat(ExtPlayer player, string message)
        {
            try
            {
                ...
                message = (characterData.AdminLVL >= 6) ? $"{ChatColors.AdminChat}[A] {ChatColors.HAdmin}{player.Name}{ChatColors.AdminChat} ({characterData.UUID}): {message}" : $"{ChatColors.AdminChat}[A] {ChatColors.LAdmin}{player.Name}{ChatColors.AdminChat} ({characterData.UUID}): {message}";
        ...
        public static void adminGlobal(ExtPlayer player, string message)
        {
            try
            {
                ...
                if (characterData.AdminLVL <= 6)
                {
                    ...
                    {
                        Trigger.SendToAdmins(1, $"{ChatColors.Red}[A] {player.Name} ({characterData.UUID}) был снят системой за причину в глобале: {message}");
                        Character.BindConfig.Repository.DeleteAdmin(player);
                        return;
                    }
                }
                ...
                else
                {
                    ...
                    Trigger.SendToAdmins(2, "!{#FFB833}" + $"[A] Запрос от {player.Name} ({characterData.UUID}) на global({message}). Чтобы подтвердить действие - введите: /accept {GlobalID}");
                    GlobalID++;
                }
            }
            catch (Exception e)
        ...
        public static void adminGlobalAccept(ExtPlayer player, int id)
        {
            try
            {
                ...
                AdminsLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) подтвердил запрос на global({GlobalQueue[id].Item2})");
        ...
        public static void sendPlayerToDemorgan(ExtPlayer admin, ExtPlayer target, int time, string reason)
        {
            try
            {
                ...
                Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{admin.Name} посадил в деморган {target.Name}({targetCharacterData.UUID}) на {time} {deTimeMsg}. Причина: {reason}", target);
                ...
                //
                target.Position = new Vector3(1688.237, 2601.2024, 44.44048) + new Vector3(0, 0, 1.5);
        ...
        public static void releasePlayerFromDemorgan(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} выпустил из деморгана {target.Name} ({targetCharacterData.UUID})");
