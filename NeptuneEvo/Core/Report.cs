    ...
    class ReportSys : Script
    {
            ...
            public void Send(ExtPlayer someone = null)
            {
                try
                {
                    if (someone == null)
                    {
                        foreach (ExtPlayer foreachPlayer in Main.AllAdminsOnline)
                        {
                            try
                            {
                                ...
                                else authorName = $"{Author}[{authorCharacterData.UUID}]";
                    ...
                    else
                    {
                        ...
                        string authorName;
                        if (authorCharacterData == null) authorName = $"{Author}[Вышел]";
                        else authorName = $"{Author}[{someoneCharacterData.UUID}]";
        ...
        public static void ReportFunc(ExtPlayer player, int ID, string funcName)
        {
            try
            {
                ...
                var targetData = player.GetCharacterData();
                ...
                else
                {
                    ...
                    {
                        case "tp":
                            Commands.CMD_teleport(player, targetData.UUID);
                            return;
                        case "metp":
                            Admin.teleportTargetToPlayer(player, target);
                            return;
                        case "sp":
                            AdminSP.Spectate(player, targetData.UUID);
                            return;
                        case "stats":
                            Commands.CMD_showPlayerStats(player, targetData.UUID);
                            return;
                        case "kill":
                            Admin.killTarget(player, target);
                            return;
                        case "ptime":
                            Commands.CMD_pcheckPrisonTime(player, targetData.UUID);
                            return;
                        case "checkdim":
                            Commands.CMD_checkDim(player, targetData.UUID);
                            return;
                        case "nhistory":
                            Commands.CMD_NickHistory(player, targetData.UUID);
                            return;
                        default:
        ...
        private static void AddAnswer(ExtPlayer player, int repID, string response)
        {
            try
            {
                ...
                else
                {
                    ...
                    Trigger.SendChatMessage(target, $"{ChatColors.Report}Администратор {player.Name} ({characterData.UUID}): {response}");
                    ...
                    Trigger.SendToAdmins(Main.ServerSettings.MinAdminLvlReport, $"{ChatColors.Report}[A][{responsetime.ToString("mm\\:ss")}] {player.Name}({characterData.UUID}) для {target.Name}({targetData.UUID}): {response}");
                }
