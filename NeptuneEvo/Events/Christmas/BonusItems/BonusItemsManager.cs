    ...
    public class BonusItemsManager
    {
        ...
        public static void OnEveryMinute()
        {
            try
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    var bonusItem = disabledItems[Main.rnd.Next(0, disabledItems.Count)];
                    bonusItem.Spawn();

                    disabledItems.Remove(bonusItem);
                }
            }
            catch (Exception ex)
            {
        ...
        public static void OnInteraction(ExtPlayer player, int index)
        {
            try
            {
                ...
                {
                    Log.Write($"OnInteraction: Invalid index {index}. Pool size: {Pool.Count}");
                    return;
                }
