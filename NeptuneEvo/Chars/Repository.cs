    ...
    class Repository : Script
    {
        ...
        public static List<RouletteCaseData> RouletteCasesData = new List<RouletteCaseData> { 
            ...
            */ 
            ...
            new RouletteCaseData(ItemId.Case11, "Легендарный кейс", 5555, new List<RouletteItemData>(), "keys_11", "Все или ничего! Хочешь рискнуть и стать обладателем легендарной кофты Piece? Тогда крути кейс!"),
 ...
 */
        ...
        public static IReadOnlyDictionary<ItemId, ItemsInfo> ItemsInfo = new Dictionary<ItemId, ItemsInfo>()
        {
            ...
            { ItemId.Case11, new ItemsInfo(RouletteCasesData[11].Name, "Все или ничего! Хочешь рискнуть и стать обладателем легендарной кофты Piece? Тогда крути кейс!", "inv-item-case11", "Особое", NAPI.Util.GetHashKey("prop_idol_case_02"), 100, new Vector3(0.0,0.0,-1.0), new Vector3(90, 0, 0), newItemType.Cases) },
            ...
            { ItemId.Metaldetector, new ItemsInfo("Металлоискатель", "Металлоискатель описание", "inv-item-metaldetector", "Особое", NAPI.Util.GetHashKey("w_am_metaldetector"), 1, new Vector3(0.0,0.0,-0.92), new Vector3(90, 0, 0), newItemType.None) },
            { ItemId.Shovel, new ItemsInfo("Лопата", "Лопата описание", "inv-item-shovel", "Остальное", NAPI.Util.GetHashKey("prop_tool_shovel2 "), 1, new Vector3(0, 0, -0.99), new Vector3(90, 0, 0), newItemType.None) },
        ...
        public static void UpdatePlayerHpArmor(ExtPlayer player)
        {
            try
            {
                if (!player.IsCharacterData()) return;
                ...
                Trigger.ClientEvent(player, "client.inventory.UpdateHpArmor", hp, armor);
            }
            catch (Exception e)
            {
                Log.Write($"UpdatePlayerHpArmor Exception: {e}");
            }
        }
                ...
                switch (SlotId)
                {
                    ...
                    case 7:
                        ...
                        if (Item.ItemId != ItemId.Debug && Variation == 0)
                        {
                            ...
                            {
                                Variation = 12; Texture = 1;
                            }

                            else
                            {
                                var fractionData = Manager.GetFractionData(memberFractionData.Id);
                                if (fractionData.Id == 1) { Variation = 28; Texture = 0; }
                                else if (fractionData.Id == 2) { Variation = 28; Texture = 2; }
                                else if (fractionData.Id == 3) { Variation = 28; Texture = 1; }
                                else if (fractionData.Id == 4) { Variation = 28; Texture = 5; }
                                else if (fractionData.Id == 5) { Variation = 28; Texture = 4; }
                                else if (fractionData.Id == 6) { Variation = 28; Texture = 8; }
                                else if (fractionData.Id == 7) { Variation = 28; Texture = 9; }
                                else if (fractionData.Id == 8) { Variation = 28; Texture = 7; }
                                else if (fractionData.Id == 9) { Variation = 28; Texture = 6; }
                                else if (fractionData.Id == 10) { Variation = 28; Texture = 7; }
                                else if (fractionData.Id == 11) { Variation = 28; Texture = 7; }
                                else if (fractionData.Id == 12) { Variation = 28; Texture = 7; }
                                else if (fractionData.Id == 13) { Variation = 28; Texture = 7; }
                                else if (fractionData.Id == 14) { Variation = 15; Texture = 0; }
        ...
        public void onResourceStart()
        {
            try
            {
                Log.Write("Система ХП и Армор загружена " + "\u001B[33m" + "[by Ale]" + "\u001B[0m");
            }
            catch (Exception e)
            {
