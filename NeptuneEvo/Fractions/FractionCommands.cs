        ...
        public static void playerPressCuffBut(ExtPlayer player)
        {
            try
            {
                ...
                if (characterData.AdminLVL != 0 || Manager.FractionTypes[fracId] == FractionsType.Gov || Manager.FractionTypes[fracId] == FractionsType.Nongov) // for gov factions or admins
                {
                    ...
                    {
                        uncuffmesp = LangFunc.GetText(LangType.Ru, DataName.YouUncuffPlayer, targetCharacterData.UUID);
                        uncuffmest = LangFunc.GetText(LangType.Ru, DataName.SomeoneUncuffsYou, characterData.UUID);
                        uncuffme = "развязал" + (characterData.Gender ? "" : "а") + " игрока {name}";
                    }
                    else
                    {
                        cuffmesp = LangFunc.GetText(LangType.Ru, DataName.NaruchnikiTiOdel, targetCharacterData.UUID);
                        cuffmest = LangFunc.GetText(LangType.Ru, DataName.NaruchnikiNaVasOdel, characterData.UUID);
                        cuffme = "надел" + (characterData.Gender ? "" : "а") + " наручники на игрока {name}"; //
                        uncuffmesp = LangFunc.GetText(LangType.Ru, DataName.ViSnyaliNaruchniki, targetCharacterData.UUID);
                        uncuffmest = LangFunc.GetText(LangType.Ru, DataName.OnSnyalNaruchniki, characterData.UUID);
                ...
                else // for mafia
                {
                    ...
                    sound = "zips";
                    cuffmesp = LangFunc.GetText(LangType.Ru, DataName.YouCuffed, targetCharacterData.UUID);
                    cuffmest = LangFunc.GetText(LangType.Ru, DataName.PlayerCuffsYou, characterData.UUID);
