    ...
    class Ems : Script
    {
        ...
        public static string OnCallEms(ExtPlayer player, bool death = false)
        {
            try
            {
                ...
                Blip EMSBlip = NAPI.Blip.CreateBlip(0, player.Position, 1, 70, $"EMS Call from [{characterData.UUID}]", 0, 0, false, 0, NAPI.GlobalDimension);
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    ...
                    Notify.Send(foreachPlayer, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.Ru, DataName.MedicCallFrom, player.Name, characterData.UUID), 5000);
                    NAPI.Chat.SendChatMessageToPlayer(foreachPlayer, "!{#F08080}" + LangFunc.GetText(LangType.Ru, DataName.MedicCallFromDistance, player.Name, characterData.UUID, Math.Round(player.Position.DistanceTo(foreachPlayer.Position), 2)));
                }
                ...
                else
                {
                    ...
                    if (!sessionData.DeathData.IsDying)
                    {
                        ...
                        {*/
                            sessionData.DeathData.InDeath = true;
                            player.SetSharedData("InDeath", true);
                        var kdata = entityKiller.GetCharacterData();
                            int medics = Manager.FractionMembersCount ((int) Models.Fractions.EMS);

                            if (entityKiller.IsCharacterData() && entityKiller != player) Trigger.ClientEvent(player, "openHospitalDialog", LangFunc.GetText(LangType.Ru, DataName.YouAreDeadFromPlayer, kdata.UUID, medics));
