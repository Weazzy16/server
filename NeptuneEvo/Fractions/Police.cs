    ...
    class Police : Script
    {
        ...
        public static string OnCallPolice(ExtPlayer player, string reason)
        {
            try
            {
                ...
                var blip = (ExtBlip)NAPI.Blip.CreateBlip(0, player.Position, 1, 70, $"Police Call from {player.Name.Replace('_', ' ')} [{characterData.UUID}]", 0, 0, true, 0, 0);
                blip.Transparency = 0;
                ...
                Manager.sendFractionMessage((int) Models.Fractions.POLICE, "!{#F08080}[F] " + LangFunc.GetText(LangType.Ru, DataName.PoliceCallFrom, player.Name, characterData.UUID, reason), true);
                //Manager.sendFractionMessage(9, $"Поступил вызов от ({player.Value}) - {reason}");
                Manager.sendFractionMessage((int) Models.Fractions.FIB, "!{#F08080}[F] " + LangFunc.GetText(LangType.Ru, DataName.PoliceCallFrom, player.Name, characterData.UUID, reason), true);
                Manager.sendFractionMessage((int) Models.Fractions.SHERIFF, "!{#F08080}[F] " + LangFunc.GetText(LangType.Ru, DataName.PoliceCallFrom, player.Name, characterData.UUID, reason), true);
