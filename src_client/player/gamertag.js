...
const SetNameTag = (player) => {
    try {
        ...
        if (player['INVISIBLE'] || player['HideNick']) {
            ...
            if (player['REDNAME']) {
                player.nameTag = `~r~Администратор сервера` + uuid;
            } else {
    ...
    {
        callRemote("client_trycatch", "player/gametag", "SetNameTag", e.toString());
    }
}
		...
		if (global.isTagsHead) {
			...
			nametags.forEach(nametag => {
				...
				catch (e) 
				{
					...
					callRemote("client_trycatch", "player/gamertag", "render 2", e.toString());
				}
			})
		}*/
	}
...
const DrawPlayerName = (player, x, y) => {
	...
	{
		callRemote("client_trycatch", "player/gametag", "DrawPlayerName", e.toString());
	}
}
...
const GetCurrentName = (player, id) => {
	...
	{
		callRemote("client_trycatch", "player/gametag", "GetCurrentName", e.toString());
		return "";
	}
}