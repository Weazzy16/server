 ...
 */
        ...
        public static IReadOnlyDictionary<ItemId, ItemsInfo> ItemsInfo = new Dictionary<ItemId, ItemsInfo>()
        {
            ...
            { ItemId.Pizza, new ItemsInfo("Пицца", "Ммм... Пицца... Восстанавливает 30% здоровья.","inv-item-pizza", "Еда", 0, 3, new Vector3(0.0,0.0,-1.0), new Vector3(), newItemType.Eat) },
            ...
            { ItemId.Case11, new ItemsInfo(RouletteCasesData[11].Name, "Все или ничего! Хочешь рискнуть и стать обладателем легендарной кофты Aria? Тогда крути кейс!", "inv-item-case11", "Особое", NAPI.Util.GetHashKey("prop_idol_case_02"), 100, new Vector3(0.0,0.0,-1.0), new Vector3(90, 0, 0), newItemType.Cases) },
            ...
            { ItemId.VehicleNumber, new ItemsInfo("Номер на автомобиль", "Номерной знак транспортного средства. Можно установить на свою машину, продать или подарить кому-нибудь!","inv-item-SimCard", "Особое", NAPI.Util.GetHashKey(" "), 1, new Vector3(0.0,0.0,-0.935), new Vector3(-90, 0, 90), newItemType.None) }, //p_num_plate_02
            ...
            { ItemId.Spank, new ItemsInfo("SPANK","Капсула с неизвестным веществом.","inv-item-medical-kit", "Наркотик", NAPI.Util.GetHashKey("p_syringe_01_s"), 100, new Vector3(0.0,0.0,-0.9), new Vector3(), newItemType.Eat) },
            ...
            { ItemId.Giftcoin, new ItemsInfo("Подарок", "Коробка с подарком, которую запрятал для тебя Хоакин Феникс!","inv-item-Assault-Rifle", "Особое", NAPI.Util.GetHashKey(""), 999, new Vector3(0.0,0.0,-0.99), new Vector3(90, 0, 0), newItemType.None) },
            ...
            { ItemId.Defibrillator, new ItemsInfo("Дефибриллятор", "Прибор, использующийся в медицине для электроимпульсной терапии нарушений сердечного ритма.","inv-item-defik", "Медицина", NAPI.Util.GetHashKey("prop_power_cell"), 1, new Vector3(0.0,0.0,-0.9), new Vector3(), newItemType.None) },

            { ItemId.Medovuha, new ItemsInfo("Медовуха", "Может сделать вас супергероем!", "inv-item-improvement", "Бафф", 1940235411, 5, new Vector3(0.0,0.0,-0.97), new Vector3(-80, 0, 0), newItemType.Improvement) },
            { ItemId.ArmorGift, new ItemsInfo("Бронежилет", "После активации - скин на бронежилет можно выбрать в Меню K", "inv-item-gift", "Особое", NAPI.Util.GetHashKey("mj_xm_box7"), 1, new Vector3(0, 0, -0.99), new Vector3(90, 0, 0), newItemType.None) },

            //НОВЫЙ ГОД
            { ItemId.Candy, new ItemsInfo("Леденцы", "Ивентовый предмет", "inv-item-event", "Остальное", NAPI.Util.GetHashKey("mj_xm_candy"), 15, new Vector3(0, 0, -0.99), new Vector3(90, 0, 0), newItemType.None) },
            { ItemId.ChristmasTreeDecoration, new ItemsInfo("Елочная игрушка", "Ивентовый предмет", "inv-item-event", "Остальное", NAPI.Util.GetHashKey("mj_xm_toy_tree6"), 15, new Vector3(0, 0, -0.99), new Vector3(90, 0, 0), newItemType.None) },
            { ItemId.ChristmasTreeStar, new ItemsInfo("Елочная звезда", "Ивентовый предмет", "inv-item-event", "Остальное", NAPI.Util.GetHashKey("prop_cs_script_bottle_01"), 15, new Vector3(0, 0, -0.99), new Vector3(90, 0, 0), newItemType.None) },

            { ItemId.SmallGift, new ItemsInfo("Маленький подарок", "Сюрприз", "inv-item-gift", "Остальное", NAPI.Util.GetHashKey("mj_xm_box7"), 1, new Vector3(0, 0, -0.99), new Vector3(90, 0, 0), newItemType.None) },
            { ItemId.MediumGift, new ItemsInfo("Средний подарок", "Сюрприз", "inv-item-gift", "Остальное", NAPI.Util.GetHashKey("mj_xm_box1"), 1, new Vector3(0, 0, -0.99), new Vector3(90, 0, 0), newItemType.None) },
            { ItemId.BigGift, new ItemsInfo("Большой подарок", "Сюрприз", "inv-item-gift", "Остальное", NAPI.Util.GetHashKey("mj_xm_box3"), 1, new Vector3(0, 0, -0.99), new Vector3(90, 0, 0), newItemType.None) },
        };
        ...
        public static void PlayerStats(ExtPlayer player, ExtPlayer target = null)
        {
            try
            {
                ...
                if (house != null)
                {
                    ...
                    charData.Add(house.Price);//46
                    charData.Add(house.Position);//46EnterPoint
                }
                else
                {
                ...
                if (targetCharacterData.BizIDs.Count > 0)
                {
                    ...
                    charData.Add(biz.SellPrice);//38
                    charData.Add(biz.EnterPoint);
                    charData.Add(biz.Type);
                }
                else
                {
                    charData.Add(null);//38
                ...
                if (targetCharacterData.WantedLVL != null)
                    ...
                    charData.Add(0);//40          
                charData.Add(targetCharacterData.EarnedMoneyDay);//41
                charData.Add(targetCharacterData.EarnedMoneyMonth);//42
                charData.Add(targetCharacterData.EarnedMoney);//43
                                                              //
                charData.Add(targetCharacterData.SpentMoneyDay);//44
                charData.Add(targetCharacterData.SpentMoneyMonth);//45
                charData.Add(targetCharacterData.SpentMoney);//46
                
                charData.Add(Reports.Count);
