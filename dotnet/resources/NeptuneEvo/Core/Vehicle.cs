...
namespace NeptuneEvo.Core
{
    class VehicleManager : Script
    {
        ...
        private static void MileageControl()
        {
            try
            {
                var moving = RAGE.Entities.Vehicles.All
                    .Cast<ExtVehicle>()
                    .Where(v => v.GetVehicleLocalData()?.EngineOn == true)
                    .ToList();

                foreach (var veh in moving)
                {
                    ...
                    // сохраняем новую позицию сразу (для следующего тика)
                    local.Position = $"{cx};{cy};{cz}";

                    // ** отфильтруем всё меньше 1 метра **
                    if (meters < 1f)
                        continue;

                    // теперь целые метры (floor, чтобы не перепрыгнуть)
                    var m = (int)Math.Floor(meters);
                    data.Mileage += m;
                    veh.SetSharedData("MILEAGE", data.Mileage);
                }
            }
            catch (Exception e)
            {
        ...
        public static async Task<int> SaveSql(ServerBD db, string number)
        {
            try
            {
                ...
                if (vehicleData == null)
                {
                    Log.Write($"SaveSql: no vehicleData for plate {number}");
                    return 0;
                }
                ...
                Log.Write($"SaveSql: updated {affected} rows for plate {number} (mileage={vehicleData.Mileage})");
                return affected;
            }
            catch (Exception e)
            {
                Log.Write($"SaveSql Exception for plate {number}: {e}");
                return 0;
            }
        }
