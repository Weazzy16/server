...
namespace NeptuneEvo.Core
{
    public class VehicleStreaming : Script
    {
        ...
        public async Task UpdateMileage(ExtPlayer player, int vehicleId, int centimeters)
        {
            ...
            // накапливаем сантиметры
            local.Centimeters += (uint)centimeters;

            // целые километры (100 000 см = 1 км), без округлений
            var addKm = (int)(local.Centimeters / 100000u);
            if (addKm > 3) addKm = 3;      // ограничитель, по желанию
            if (addKm <= 0) return;        // если не накопился хотя бы 1 км — уходим

            // добавляем к пробегу и убираем из сантиметров полные километры
            local.Mileage += addKm;
            local.Centimeters -= (uint)(addKm * 100000);

            

            // синхронизируем в оперативной памяти сервера
            ...
            var rows = await VehicleManager.SaveSql(db, local.NumberPlate);
